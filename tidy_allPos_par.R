...
tidy_add_team_data<-function(yards){
  ...
  registerDoSNOW(cl)
  tic("sleeping")
  print("determine which team they played the most season with")
...
tidy_reduce_data <-function(yards){
  print("reducing to players with at leat 5 years of 8 games or more")
...
tidy_fix_flipped_data<-function(yards){
  print("fix the flipped completios and attempts")
...
tidy_win_credit<-function(yards){
  print("determin who gets credit for win - only works for QB at moment")
  #set default at no
